...
st.markdown("""
    ...
    /* Removed fragile Streamlit CSS targeting - using JS injection below instead */

    /* METRIC CONTAINER SIZING & BORDERS */
    /* 1. Apply Green Border to the exact outer container */
    div[data-testid="stTabs"] div[data-testid="stColumn"] > div[data-testid="stVerticalBlock"] > div:has(p.metric-label) {
        border-color: #1a7b6b !important;
    ...
    /* 2. STRIP the grey inner border generated by Streamlit's st.container(border=True) */
    div[data-testid="stTabs"] div[data-testid="stColumn"] > div[data-testid="stVerticalBlock"] > div:has(p.metric-label) div {
        border: none !important;
        background-color: transparent !important;
    }

    /* 3. Stack the text vertically */
    div[data-testid="stTabs"] div[data-testid="stColumn"] > div[data-testid="stVerticalBlock"] > div:has(p.metric-label) > div[data-testid="stVerticalBlock"] {
        gap: 0 !important;
        padding: 0 !important;
        display: flex !important;
        flex-direction: column !important;
    ...
    div[data-testid="stTabs"] div[data-testid="stColumn"] > div[data-testid="stVerticalBlock"] > div:has(p.metric-label) .element-container {
    ...
    div[data-testid="stTabs"] div[data-testid="stColumn"] > div[data-testid="stVerticalBlock"] > div:has(p.metric-label):hover {
        background-color: #1a7b6b !important;
    }
    
    /* Invert text to white on hover using a universal selector inside the container */
    div[data-testid="stTabs"] div[data-testid="stColumn"] > div[data-testid="stVerticalBlock"] > div:has(p.metric-label):hover p,
    div[data-testid="stTabs"] div[data-testid="stColumn"] > div[data-testid="stVerticalBlock"] > div:has(p.metric-label):hover span,
    div[data-testid="stTabs"] div[data-testid="stColumn"] > div[data-testid="stVerticalBlock"] > div:has(p.metric-label):hover div {
        color: white !important;
    }
...
# Bulletproof styling injector for the summary tables (immune to Streamlit DOM changes)
import streamlit.components.v1 as components
components.html("""
<script>
    const docs = window.parent.document;
    function applyStyles() {
        // 1. Style Outer Containers (Green Border + Green HRs)
        docs.querySelectorAll('.summary-table-marker').forEach(marker => {
            let el = marker;
            while (el && el !== docs.body) {
                if (el.classList.contains('summary-processed')) break;
                let style = window.getComputedStyle(el);
                // Find the container with a border
                if (style.borderWidth !== '0px' && style.borderStyle !== 'none' && el.tagName === 'DIV') {
                    el.style.borderColor = '#1a7b6b';
                    el.style.borderWidth = '2px';
                    el.style.borderStyle = 'solid';
                    el.classList.add('summary-processed');
                    
                    // Style HRs inside it
                    el.querySelectorAll('hr').forEach(hr => {
                        hr.style.borderTop = '1px solid #1a7b6b';
                        hr.style.opacity = '1';
                    });
                    break;
                }
        ...
        // 2. Style Inner Scroll Containers (Remove Border + Max Height)
        docs.querySelectorAll('.scroll-table-marker').forEach(marker => {
            let el = marker;
            while (el && el !== docs.body) {
                if (el.classList.contains('scroll-processed')) break;
                let style = window.getComputedStyle(el);
                // Find the scroll container or border container
                if (style.height === '600px' || (style.borderWidth !== '0px' && style.borderStyle !== 'none')) {
                    el.style.border = 'none';
                    el.style.height = 'auto';
                    el.style.maxHeight = '600px';
                    el.style.overflowY = 'auto';
                    el.classList.add('scroll-processed');
                    break;
                }
                el = el.parentElement;
            }
